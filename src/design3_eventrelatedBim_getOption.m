function opt = design3_eventrelatedBim_getOption()
    %   __  ____  ____     _      _    _
    %  / _)(  _ \(  _ \   | |    / \  | )
    % ( (_  )___/ )___/   | |_  / _ \ | \
    %  \__)(__)  (__)     |___||_/ \_||__)
    ...
    root_dir = spm_file(fullfile(this_dir, '..', '..', '..'), 'cpath');

    opt.dataDir = fullfile(root_dir, 'sourcedata');
    assert(isfolder(opt.dataDir))
    opt.derivativesDir = fullfile(root_dir, '..', '..');
    assert(isfolder(opt.derivativesDir))

    % specify the model file that contains the contrasts to compute
    % at the univariate level
    % (but to be used later for multivariate analyses)
    opt.model.file = fullfile(this_dir, 'model-eventrelatedBimMultivariateActor_smdl.json');
    ...
    opt.jobsDir = fullfile(opt.dataDir, ...
                           '..', ...
                           'derivatives', ...
                           'SPM12_CPPL', ...
                           'JOBS', ...
