...
function cosmomvpaSphereRoiCrossValidation(opt, funcFWHM)
...
pathLoc = derivativesDir; % opt.locDir;
pathROI = opt.maskDir;
extLoc = opt.derivExtension;

% set output dir
pathOutput = fullfile(derivativesDir,extLoc,opt.resDir);
...
%for iSub = 1:length(subjects)
        ...
        pathData = getFFXdir(subID, funcFWHM, opt);
...
%                 end
                ...
                opt.cosmomvpa.feature_selection_ratio_to_keep = opt.cosmomvpa.ratioToKeep(iRoi);
                
                % load ROI masks for a specific dimension
                roiPattern = fullfile(strcat(pathLoc, extLoc,'sub-',subID,pathROI,'/sub-',subID,'_',opt.cosmomvpa.rois(iRoi),'mask_10mm_res',num2str(funcFWHM),'.nii'));
